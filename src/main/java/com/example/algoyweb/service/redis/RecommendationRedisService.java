package com.example.algoyweb.service.redis;
...
public class RecommendationRedisService {
    ...
    /**
     * 추천 refresh lock의 유지 시간을 분 단위로 보관한다.
     * 같은 사용자의 중복 refresh를 일정 시간 동안 막기 위한 TTL 값이다.
     * 작업 중 서버가 비정상 종료되더라도 lock이 영구히 남지 않도록 한다.
     */
    @Value("${recommendation.refresh-lock.ttl-minutes}")
    private long recommendationRefreshLockTtlMinutes;
    ...
    @Value("${recommendation.refresh.threshold:3}")
    private long recommendationRefreshThreshold;
...
//    }
    ...
    public boolean needsRefresh(String userEmail) {
        ...
        return getActiveRecommendationCount(userEmail) <= recommendationRefreshThreshold;
    }
    ...
    /**
     * 사용자별 추천 refresh lock 키를 생성
     *
     * @author 조아라
     * @since 2026.04.23
     *
     * 전역 lock이 아니라 사용자 단위 lock을 사용해서
     * 같은 사용자에 대한 중복 refresh만 막고, 다른 사용자의 refresh는 병렬로 허용한다.
     * 예: recommendation:refresh-lock:user@example.com
     */

    //사용자별 lock 키 이름을 만듭니다.
    private String refreshLockKey(String userEmail) {
        return "recommendation:refresh-lock:" + userEmail;
    }

    /**
     * 사용자별 추천 refresh lock 획득을 시도
     *
     * @author 조아라
     * @since 2026.04.23
     *
     * Redis의 setIfAbsent(SETNX) + TTL 방식으로 동작하며,
     * 이미 다른 요청이 같은 사용자의 refresh를 수행 중이면 false를 반환
     * 반환값:
     *  - true: 현재 요청이 refresh 작업을 수행해도 됨
     *  - false: 다른 요청이 이미 refresh 중이므로 중복 실행을 피해야 함
     */
    // Redis에 "키가 없을 때만" lock을 잡습니다.
    public boolean tryAcquireRefreshLock(String userEmail) {
        Boolean result = stringRedisTemplate.opsForValue().setIfAbsent(
                refreshLockKey(userEmail),
                "1",
                Duration.ofMinutes(recommendationRefreshLockTtlMinutes)
        );
        return Boolean.TRUE.equals(result);
    }

    /**
     * 사용자별 추천 refresh lock을 해제한다.
     *
     * @author 조아라
     * @since 2026.04.23
     *
     * refresh 작업이 정상적으로 끝난 뒤 lock을 제거해서
     * 이후 요청이 다시 refresh를 수행
     * 보통 refresh 작업의 finally 블록에서 호출하는 것이 안전
     *
     */
    public void releaseRefreshLock(String userEmail) {
        stringRedisTemplate.delete(refreshLockKey(userEmail));
    }
}
